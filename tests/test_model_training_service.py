...
def test_run_training_job_forwards_fee_and_slippage_from_training_profile(monkeypatch, tmp_path):
    csv_path = tmp_path / "features.csv"
    csv_path.write_text("timestamp,close\n2026-01-01T00:00:00Z,100\n", encoding="utf-8")
    ...
    def _fake_train_and_evaluate_model(*args, **kwargs):
        captured["fee_per_trade"] = kwargs.get("fee_per_trade")
    ...
    result = model_training_service_module.run_training_job(
        ...
        candidate_top_n=5,
        candidate_fresh_ratio=0.3,
        training_profile={"n_splits": 3, "fee_per_trade": 2.5, "slippage_bps": 4.0},
    )

    assert captured["fee_per_trade"] == 2.5
    assert captured["slippage_bps"] == 4.0
    assert captured["meta_extra"]["fee_per_trade"] == 2.5
    assert captured["meta_extra"]["slippage_bps"] == 4.0
    assert result["status"] == "ok"
